...
namespace DominusCore {
	public class Game : GameWindow {
		...
		protected override void OnRenderThreadStarted() {
			...
			GeometryShader = new ShaderProgramGeometry(ShaderProgram.CreateShader("src/GeometryShader-Vertex.glsl", ShaderType.VertexShader), ShaderProgram.CreateShader("src/GeometryShader-Fragment.glsl", ShaderType.FragmentShader)).use();
			LightingShader = new ShaderProgramLighting(ShaderProgram.CreateShader("src/LightingShader-Vertex.glsl", ShaderType.VertexShader), ShaderProgram.CreateShader("src/LightingShader-Fragment.glsl", ShaderType.FragmentShader)).use();
		...
		private void ConstructScene() {
			...
			for (int x = 0; x < xMax; x++) {
				for (int y = 0; y < yMax; y++) {
					...
					vertices.AddRange(new float[] { x / xMax, y / yMax });
					vertices.AddRange(new float[] { 1.0f, 1.0f, 1.0f, 1.0f, 1.0f, 1.0f, 1.0f }); //color and normal
				}
			}
			Model landscape = new Model(vertices.ToArray(), indices.ToArray(), tilesTextures).SetPosition(new Vector3(20, 5, 6)).SetScale(20f);

			SceneRoot = new Drawable();
			SceneRoot.AddChildren(plane, lightCube, circle, landscape);
